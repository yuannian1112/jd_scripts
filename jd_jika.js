...
function cardHomePage() {
    return new Promise(resolve => {
        $.post(taskPostUrl('necklacecard_cardHomePage', `body={"activityKey": "${$.activityKey}"}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
async function taskList() {
    return new Promise(resolve => {
        $.post(taskPostUrl('necklacecard_taskList', `body={"activityKey": "${$.activityKey}"}`), async (err, resp, data) => {
            try {
                if (err) {
...
function doTask(activityKey, encryptTaskId, itemId) {
    return new Promise(resolve => {
        $.post(taskPostUrl('necklacecard_taskReport', `body={"activityKey": "${activityKey}","encryptTaskId": "${encryptTaskId}","itemId": "${itemId}"}`), async (err, resp, data) => {
            try {
                if (err) {
                    data = JSON.parse(resp.body)
                    console.log(`${data.error.msg}`)
...
function openCard() {
    return new Promise(resolve => {
        $.post(taskPostUrl('necklacecard_openCard', `body={"activityKey": "${$.activityKey}"}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                    ...
                    if (safeGet(data)) {
                        data = JSON.parse(data);
                        if (data.data.result) {
                            console.log(`抽卡成功，获得【${data.data.result.cardName}】\n`)
                        }
                    }
                }
            } catch (e) {
