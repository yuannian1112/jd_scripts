...
[task_local]
#京喜财富岛月饼
5 * * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js, tag=京喜财富岛月饼, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "5 * * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js,tag=京喜财富岛月饼

===============Surge=================
京喜财富岛月饼 = type=cron,cronexp="5 * * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js

============小火箭=========
京喜财富岛月饼 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_cfd.js, cronexpr="5 * * * *", timeout=3600, enable=true
 */
                    ...
                    switch (type) {
                        ...
                        case 3:
                            ...
                            if (data.iRet === 0) {
                                ...
                                if (data.strDT) {
                                    let count = $.isNode() ? (process.env.JD_CFD_RUNNUM ? process.env.JD_CFD_RUNNUM * 1 : Math.floor((Math.random() * 5)) + 5) : ($.getdata('JD_CFD_RUNNUM') ? $.getdata('JD_CFD_RUNNUM') * 1 : Math.floor((Math.random() * 5)) + 5);
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        if ($.isNode()) {
            ...
            console.log(`\nBoxJs设置的京喜财富岛月饼邀请码:${$.getdata('jd_jxCFD')}\n`);
        }
