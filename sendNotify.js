...
// =======================================MYQQ设置区域=======================================
let MYQQ_API = '';		//请求API
let MYQQ_token = '';	//后台设置的token
let MYQQ_robot = '';	//机器人QQ
let MYQQ_type = '';		//请求类型	1好友 2群 3讨论组 4群临时会话 5讨论组临时会话 6在线临时会话
let MYQQ_user = '';		//接收人QQ
let MYQQ_group = '';	//接收群号

//==========================云端环境变量的判断与接收=========================
if (process.env.MYQQ_API) {
  MYQQ_API = process.env.MYQQ_API;
}

if (process.env.MYQQ_token) {
  MYQQ_token = process.env.MYQQ_token;
}

if (process.env.MYQQ_robot) {
  MYQQ_robot = process.env.MYQQ_robot;
}

if (process.env.MYQQ_type) {
  MYQQ_type = process.env.MYQQ_type;
}

if (process.env.MYQQ_user) {
  MYQQ_user = process.env.MYQQ_user;
}
if (process.env.MYQQ_group) {
  MYQQ_group = process.env.MYQQ_group;
}
...
function myqq(text, desp, time = 2100) {
    ...
    if (GOBOT_URL) {
      const options = {
        ...
        json: {
			  ...
			  "token": `${MYQQ_token}`,            //后台设置的token
			  "params": {
				  "c1": `${MYQQ_robot}`,            //参数1，要使用的机器人QQ
				  "c2": `${MYQQ_type}`,                    //参数2，消息类型，2为群，以此类推...
				  "c3": `${MYQQ_group}`,             //参数3，要发送的群号，以此类推...
				  "c4": `${MYQQ_user}`,                     //参数4，要发送的QQ，此处发的是群，所以这个要留空，以此类推...
				  "c5": `${text}\n${desp}`       //参数5，要发送的消息内容，以此类推...
				  }
				  },
        headers: {
      ...
      setTimeout(() => {
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              console.log('发送MYQQ通知调用API失败！！\n');
              console.log(err);
            } else {
              data = JSON.parse(data);
              if (data.retcode === 0) {
                console.log('MYQQ发送通知消息成功🎉\n');
              } else if (data.retcode === 100) {
                console.log(`MYQQ发送通知消息异常: ${data.errmsg}\n`);
              } else {
                console.log(
                  `MYQQ发送通知消息异常\n${JSON.stringify(data)}`,
                );
